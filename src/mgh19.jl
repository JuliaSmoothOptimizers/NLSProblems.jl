...
function mgh19()
  ...
  y = [1.366; 1.191; 1.112; 1.013; 0.991; 0.885; 0.831; 0.847; 0.786; 0.725;
       0.746; 0.679; 0.608; 0.655; 0.616; 0.606; 0.602; 0.625; 0.651; 0.724;
  ...
  F(x) = [y[i] - x[1]*exp(-t[i]*x[5]) + x[2]*exp(-(t[i]-x[9])^2*x[6]) + x[3]*exp(-(t[i]-x[10])^2*x[7]) + x[4]*exp(-(t[i]-x[11])^2*x[8]) for i = 1:65]
  x0 = [1.3; 0.65; 0.65; 0.7; 0.6; 3.0; 5.0; 7.0; 2.0; 4.5; 5.5]

  #return SimpleNLSModel(x0, 2, F=F)
  return ADNLSModel(F, x0, 65, name="mgh19")
end
